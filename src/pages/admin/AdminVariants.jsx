  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-[#0a0a0a]">Types de variantes globaux</h2>
      ...
      {/* Form ajout */}
      {adding && (
        <div className="flex gap-2 p-3 admin-card border border-[#D7A12B]/25 rounded-xl">
          <input
            type="text"
            value={newName}
            ...
            autoFocus
            className="flex-1 px-3 py-2 bg-neutral-50 border border-black/[0.12] rounded-lg text-sm text-[#0a0a0a] placeholder-[#555] focus:border-[#D7A12B] focus:outline-none"
          />
          <button onClick={handleAdd} className="px-4 py-2 bg-[#D7A12B] text-[#0a0a0a] rounded-lg text-sm font-semibold">
            Créer
          </button>
          <button onClick={() => setAdding(false)} className="px-3 py-2 text-neutral-400 hover:text-[#0a0a0a]">
            <X size={16} />
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {types.map(t => (
            <div key={t.id} className="flex items-center justify-between px-4 py-3 admin-card border border-black/[0.08] rounded-xl group">
              {editId === t.id ? (
                <div className="flex items-center gap-2 flex-1 mr-2">
                  <input
                    ...
                    autoFocus
                    className="flex-1 px-2 py-1 bg-neutral-50 border border-[#D7A12B]/40 rounded text-sm text-[#0a0a0a] focus:outline-none"
                  />
                  <button onClick={() => handleRename(t.id)} className="text-[#D7A12B]"><Check size={15} /></button>
                  <button onClick={() => setEditId(null)} className="text-neutral-400"><X size={15} /></button>
                </div>
              ) : (
              ...
              {editId !== t.id && (
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-1.5 text-neutral-400 hover:text-[#D7A12B] rounded-lg hover:bg-[#D7A12B]/10 transition-all"
                  >
                    <Pencil size={13} />
                  </button>
                  <button
                    onClick={() => handleDelete(t.id, t.name)}
                    className="p-1.5 text-neutral-400 hover:text-red-400 rounded-lg hover:bg-red-500/10 transition-all"
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
